...
LiteralInstruction {
    ...
    private Instruction21h(Opcode opcode, byte[] arrby, int n) {
        super(opcode);
        this.regA = arrby[n + 1];
        this.litB = NumberUtils.decodeShort(arrby, n + 2);
    }

    /* synthetic */ Instruction21h(Opcode opcode, byte[] arrby, int n, 1 var4) {
    ...
    implements Instruction.InstructionFactory {
        ...
        public Instruction makeInstruction(DexFile dexFile, Opcode opcode, byte[] arrby, int n) {
            return new Instruction21h(opcode, arrby, n, null);
        }
    }
