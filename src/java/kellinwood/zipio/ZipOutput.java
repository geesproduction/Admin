...
public class ZipOutput {
    ...
    public ZipOutput(String string2) throws IOException {
        this.outputFilename = string2;
        ZipOutput.super.init(new File(this.outputFilename));
    }
    ...
    public void write(ZioEntry zioEntry) throws IOException {
        String string2 = zioEntry.getName();
        if (this.namesWritten.contains((Object)string2)) {
            ZipOutput.getLogger().warning("Skipping duplicate file in output: " + string2);
            return;
        } else {
            zioEntry.writeLocalEntry((ZipOutput)this);
            this.entriesWritten.add((Object)zioEntry);
            this.namesWritten.add((Object)string2);
