...
public class HexEncoder {
    ...
    public int decode(String string2, OutputStream outputStream) throws IOException {
        ...
        do {
            if (n2 <= 0 || !HexEncoder.super.ignore(string2.charAt(n2 - 1))) break;
            --n2;
        } while (true);
        int n3 = 0;
        while (n3 < n2) {
            int n4;
            int n5;
            for (n4 = n3; n4 < n2 && HexEncoder.super.ignore(string2.charAt(n4)); ++n4) {
            }
            byte[] arrby = this.decodingTable;
            int n6 = n4 + 1;
            byte by = arrby[string2.charAt(n4)];
            for (n5 = n6; n5 < n2 && HexEncoder.super.ignore(string2.charAt(n5)); ++n5) {
            }
            byte[] arrby2 = this.decodingTable;
            n3 = n5 + 1;
            outputStream.write(arrby2[string2.charAt(n5)] | by << 4);
            ++n;
        }
        return n;
    }
    ...
    public int decode(byte[] arrby, int n, int n2, OutputStream outputStream) throws IOException {
        ...
        int n4 = n + n2;
        do {
            if (n4 <= n || !HexEncoder.super.ignore((char)arrby[n4 - 1])) break;
            --n4;
        } while (true);
        int n5 = n;
        while (n5 < n4) {
            int n6;
            int n7;
            for (n6 = n5; n6 < n4 && HexEncoder.super.ignore((char)arrby[n6]); ++n6) {
            }
            byte[] arrby2 = this.decodingTable;
            int n8 = n6 + 1;
            byte by = arrby2[arrby[n6]];
            for (n7 = n8; n7 < n4 && HexEncoder.super.ignore((char)arrby[n7]); ++n7) {
            }
